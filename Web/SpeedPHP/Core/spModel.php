...
class spModel {
	...
	public function create($row)
	{
		...
		if( FALSE != $this->_db->exec($sql) ){ // 获取当前新增的ID
			if( $newinserid = $this->_db->newinsertid() ){
				...
				$data=$this->find($row, "{$this->pk} DESC",$this->pk);
	...
	private function __prepera_format($rows)
	{
		...
		foreach( $columns as $col ){
			$newcol[lcfirst($col['Field'])] = $col['Field'];
		}
...
class spPager {
	...
	private function runpager($func_name, $func_args){
		...
		if('findSql'==$func_name){
            $result = $this->model_obj->findSql("SELECT COUNT({$this->model_obj->pk}) as sp_counter FROM ($conditions) sp_tmp_table_pager1");
            $re = array_pop($result);
			$total_count = array_pop($re);
		}else{
