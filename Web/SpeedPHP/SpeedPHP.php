...
// 支持controller子模块
$pos = strpos($__controller, '/');
if($pos){
	$array = explode('/', $__controller);
	$__module = $array[0];
	$__controller = $array[1];
}

// 改造默认路由
// 兼容原有路由加载模式
// @FIX 待删除
if(strpos($__controller, '_controller')){
	$__controller = str_replace('_controller', '', $__controller);
}
$__controller = ucfirst($__controller);
